...
if (rfid.isCard()) {
    ...
    if (rfidCard == ""){
      ...
      Serial.write(String(rfidCard).c_str());
//      Serial.println();
       ...
       if (rfid.readCardSerial()) {
          ...
          if(rfidCard == (String(rfid.serNum[0])  + String(rfid.serNum[1])  + String(rfid.serNum[2])  + String(rfid.serNum[3])) ){
             //do nothing
             
            }else{
              rfidCard = String(rfid.serNum[0])  + String(rfid.serNum[1])  + String(rfid.serNum[2])  + String(rfid.serNum[3]);
//              Serial.println(rfidCard1);
  ...
  if (Serial.available() > 0   ) //check if the arduino is receiving data or not
{
  command=Serial.read(); 
   ...
   if(command =='F' || command =='B' || command =='L' || command =='R' || command =='S' || command =='O' || command =='K'    ){
    if (command == 'F')//moving Forward
...
else if(command == 'B') // moving backward
  {
//    Serial.println("B");
...
else if (command == 'R') //moving right
{
//  Serial.println("R"); 
    digitalWrite(IN1, HIGH);
    digitalWrite(IN2, LOW);
...
else if (command == 'O') //moving right
{
//  Serial.println("R"); 
    digitalWrite(IN1, HIGH);
    digitalWrite(IN2, LOW);
...
else if (command == 'S') //stop the motors
{ 
//  Serial.println("S");
    digitalWrite(IN1, HIGH);
    digitalWrite(IN2, HIGH);
